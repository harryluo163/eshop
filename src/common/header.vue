        ...
        <div class="w-box">
          <div class="nav-logo">
            <h1 @click="changePage(1)">
              <router-link to="/" title="花铺商城官网">花铺商城</router-link>
            </h1>
          </div>
          <div class="right-box">
            <div class="nav-list">
              ...
              <!-- <input type="button" value="搜索" name="sou" class="in2" style="cursor:pointer" @click="search()" /> -->
            ...
            <div class="nav-aside" ref="aside" :class="{fixed:st}">
              <div class="user pr">
                ...
                <a @click="to_user">个人中心</a>
                <!--用户信息显示-->
                <div class="nav-user-wrapper pa" v-if="this.login">
                  <div class="nav-user-list">
                    <ul>
                      <!--头像-->
                      <!-- <li class="nav-user-avatar" :style="{backgroundImage:'../assets/logo.png'}"> -->
                      <li class="nav-user-avatar" >
                        <div>
                          <!-- <span class="avatar" :style="{backgroundImage:url('+../assets/logo.png+')}"></span> -->
                          ...
                          <li class="clearfix" v-for="(item,i) in cartList" :key="i">
                            <div class="cart-item">
                              <div class="cart-item-inner">
                                ...
                                <div class="del-btn" @click="delGoods(item.productid)"></div>
                              </div>
                            </div>
                          </li>
                        </ul>
          ...
          <div class="nav-sub-wrapper" :class="{fixed:st}">
            <div class="w">
              <ul class="nav-list2">
                ...
                <li>
                  <router-link to="/goods"><a @click="changePage(2)" :class="{active:choosePage===2}">全部</a></router-link>
                </li>
              </ul>
              <div></div>
            </div>
...
import { getcarts, cartDel, getQuickSearch, productHome } from "../server/shop";
...
export default {
  ...
  computed: {
    ...mapState(["cartList", "receiveInCart", "showCart", "userInfo"]),
    login() {
...
//  }},
  methods: {
    ...
    // 搜索框提示
    loadAll() {
      getQuickSearch({name:this.input}).then(res => {
      ...
      getcarts({ userid: this.userid }).then(res => {
        if (res.code === 200) {
          ...
          for (var i = 0; i < goods.length; i++) {
            var item = goods[i];
            let num = item.count;
            productHome({ productid: item.productid }).then(res => {
              if (res.code === 200) {
                //  console.log('  11  ' ,num)
                var result = JSON.parse(res.data)[0];
                this.pictureurl =
                  this.serverimagepath +
                  (result.pictureurl || "").split(";")[0];
    ...
    delGoods(productid) {
      if (this.login) {
        // 登陆了
        //  console.log('商品编号',productid)
        cartDel({ userid: this.userid, productid }).then(res => {
          console.log(res);
          console.log('before',this.cartList)
          this.EDIT_CART({productid:productid});
          console.log('after',this.cartList)
          // this.reload();
          // location.reload()
        });
      }
...
.top {
  background: url("../assets/images00/top.jpg") no-repeat;
  width: 1330px;
  height: 80px;
  margin: 0 auto;
}
...
.w-box {
  ...
  // position: relative;
  background: rgba(255, 192, 187, 1);
  h1 {
    height: 100%;
    display: flex;
    align-items: center;
    > a {
      background-image: url("../assets/images00/duolei.png");
  ...
  .user {
    ...
    > a {
      ...
      &:before {
        ...
        background: url("../assets/images00/duolei.png") -200px -285px;
    ...
    .nav-user-avatar {
      > div {
        ...
        .avatar {
          background-image: url("../assets/images00/zb8.jpg");
          border-radius: 50%;
          display: block;
          @include wh(100%);
          background-repeat: no-repeat;
