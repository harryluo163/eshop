                    ...
                    <div class="cart-items clearfix">
                      ...
                      <div class="name hide-row fl">
                        <div class="name-table">
                          <a @click="goodsDetails(item.productid)" :title="item.name" target="_blank"
                             v-text="item.name"></a>
                          <!-- <ul class="attribute">
                            <li>白色</li>
                          </ul> -->
                      ...
                      <!--删除按钮-->
                      <div class="operation opation11">
                        <a class="items-delete-btn" @click="cartdel(item.productid)"></a>
                      </div>
                      <!--商品数量-->
                      <div>
                        <!--总价格-->
                        <div class="subtotal" style="font-size: 14px">¥ {{item.price * item.productNum}}</div>
                        <!--数量-->
                        ...
                        <!--价格-->
                        <div class="price1 price11">¥ {{item.price}}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="shipping">
                <div class="shipping-box">
                  ...
                  <div class="shipping-total shipping-price"><h4
                    ...
                    <h5 class="shipping-tips ng-scope">该费用不含运费</h5>
                  </div>
                </div>
...
<script>
  import { cartEdit, cartDel } from '../../server/shop'
    ...
    methods: {
      ...mapMutations([
       ...
       handleChange({value, id, checked}) {
        //  console.log(value,id,checked)
         this._cartEdit(this.userid,id,value,checked)
      ...
      editCheckAll () {
       ...
       this.cartList.forEach(item => {  
         console.log(this.userid,item.productid)
          this._cartEdit(this.userid , item.productid , item.productNum, checkAll )
      })
      ...
      _cartEdit (userid, productid, productNum, checked) {
        ...
        cartEdit( {userid:userid,productid:productid,count:productNum,check:checked}).then(res => {
          // console.log(res)
          if (res.code === 200) {
            // console.log('已修改',res)
            //修改本地
